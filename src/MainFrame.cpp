...
void MainFrame::addTaskPanel() {
    ...
    // m_mainSplitter->Add(m_taskPanel.taskPanel, wxSizerFlags().Expand().Proportion(3));
    ...
    m_taskPanel.projectNameText->SetFont(font);
...
void MainFrame::addSidebar() {
    ...
    // m_mainSplitter->Add(m_sidebar.sideBarPanel, wxSizerFlags().Expand().Proportion(1));
...
void MainFrame::onAddProjectButtonClicked(wxCommandEvent& ev) {
    ...
    m_sidebar.projectList.insert_or_assign(newProject->projectId, newProject);
